...
def cast_columns_to_float(df_spark):
    numeric_cols = ["purchase_orders_outstanding", "contract_awarded_amount", "payments_made", "remaining_contract_award_amount"]
    for col_name in numeric_cols:
        df_spark = df_spark.withColumn(col_name, col(col_name).cast("float"))
    return df_spark

def cast_columns_to_date(df_spark):
    df_spark = df_spark.withColumn("start_date", to_date(from_utc_timestamp('term_start_date', 'UTC')))
    df_spark = df_spark.withColumn("end_date", to_date(from_utc_timestamp('term_end_date', 'UTC')))
    return df_spark
...
def transform():
    ...
    try:
        with duckdb.connect('supplier.db') as conn:
            ...
            df_spark = cast_columns_to_float(df_spark)
